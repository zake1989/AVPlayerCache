...
extension CachedItemResourceLoader: FileDataDelegate {
    ...
    fileprivate func handleFetchedData(_ data: Data, at range: DataRange) {
        ...
        Cache_Print("\n fetch Range start: \(startDataRange)", level: LogLevel.resource)
        for request in rangeRequestList {
            if let dataRequest = request.dataRequest {
                Cache_Print("  fetch range : requested range \(dataRequest.currentOffset) ", level: LogLevel.resource)
                ...
                Cache_Print("  fetch range -------------------------------", level: LogLevel.resource)
                Cache_Print("  fetch range data range: \(dataRange)", level: LogLevel.resource)
                Cache_Print("  fetch range -------------------------------", level: LogLevel.resource)
                if let neededRange = dataRange.rangeClamped(startDataRange) {
                    ...
                    Cache_Print("  fetch range needed data range: \(neededRange)", level: LogLevel.resource)
                    removeRequest()
    ...
    func resourceLoader(_ resourceLoader: AVAssetResourceLoader,
        ...
        if onSingleRequestModel {
            ...
            if let dataR = loadingRequest.dataRequest {
                Cache_Print("  fetch range ===============================", level: LogLevel.resource)
                Cache_Print("  fetch range loading start range: \(dataR.requestedOffset) -- \(dataR.requestedOffset+Int64(dataR.requestedLength))", level: LogLevel.resource)
                Cache_Print("  fetch range ===============================", level: LogLevel.resource)
            }
